...
- [~] CHR-1 / CAP-1 import foundation: one bounded self-contained GLB path for upload/URL/generated results; a source-generated Mixamo mannequin, independent player/NPC/ghost instances, native idle/walk/run/fall sampling and procedural gait fallback. Takes/manifests snapshot avatar identity/tint; opt-in device storage retains exact GLBs under stable content/options IDs, restores the remembered selection and lazily prepares saved replay avatars. Storage is bounded with explicit removal and no eviction; page-only imports remain available. Full bone recording/CHR-2 crossfades and remote asset hosting remain pending (ADR-0002). Meshy text generation has a consented Worker/session-ledger adapter and resumable jobs; no paid end-to-end verification. Tripo is gated pending onboarding-clip mapping; Hunyuan is gated until its minimum 40k-face output can be reduced within the 30k import budget.
...
- [~] Local control reference package (STU-2 / GEN-2): matching beauty/depth/pose spans plus a first-frame hero PNG, camera samples, grounded per-span prompt drafts and SHA-256 file manifest in a downloadable TAR; verdict-card span controls and QA hook. No provider calls or uploads; prompt drafts do not imply endpoint conditioning support. Mapped rig-joint readback now replaces the proxy for supported Mixamo avatars, with source counts in the package; full recorded bone motion, audio, faithful generation and visual-quality acceptance remain pending (ADR-0012).
