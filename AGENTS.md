...
4. **Verify**:
   ...
   - Screenshot baselines (`tests/e2e/__baselines__/`) are Linux/SwiftShader-only; always pass `tier=desktop` in harness URLs (SwiftShader would otherwise detect as `fallback`). The picture is read off the WebGL canvas after a rendered frame (`window.__coastShot()`), once Spark's splats are a draw call (`__coastDraws ≥ 2`) — never a compositor screenshot, which lands mid-frame under SwiftShader. Regenerate with `--update-snapshots` only in a PR that explains the visual change in `art/DIFF.md`.
