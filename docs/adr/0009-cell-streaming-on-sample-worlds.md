...
- **What stays in the hub** for now: ~~props, NPCs,~~ the billboard and the missions live in the cell that was active at boot. Props, NPCs and the car became cell content in ADR-0011; zones follow `cell.json` in M2.
...
- − ~~Content does not stream with the cell yet (props, NPCs stay in the hub)~~ — superseded by ADR-0011 (content follows the cell); missions in other cells still wait for the Marble cells.
