...
The Realtime model receives: this grammar, a ≤2 KB scene summary (ids, tags, positions of salient objects; current mode/shot; active mission), and the tool schema from `packages/director/src/schema.ts`. Deictic words carry the utterance timestamp so the client resolves *what was pointed at when the word was said*.
